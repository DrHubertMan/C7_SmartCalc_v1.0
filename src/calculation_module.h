...
используемых для преобразования входной строки, содержащей выражение в инфиксной нотации,
в выходную строку, содержащую выражение в постфиксной нотации, с последующим ее вычислением.
*/
...
/*!
	\brief Макрос используется для задания максимальной длинны массива символов.
*/
#define MAX_STR 255
...
/*!
	...
	Данная структура служит для создания стека на основе односвязного списка.
  Стек используется в алгоритме сортировочной станции, для преобразования выражения в
  постфиксной нотации.
  \param value - символ обрабатываемой строки в стеке.
*/
typedef struct Node {
  char value;
  struct Node *next;
...
/*!
	...
	Данная структура служит для создания стека на основе односвязного списка.
  Стек используется для вычисления выражения записаного в строке в постфиксной нотации.
  \param value - численное значение операнда в стеке.
*/
